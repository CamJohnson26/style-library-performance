...
function App() {
  ...
  const exportMetrics = () => {
    ...
    link.href = url;
    link.download = `stitches-performance-metrics-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.json`;
    ...
    <div>
      <div style={{ 
        ...
        <h1 style={{ margin: 0, fontSize: '2.5rem', fontWeight: 'bold' }}>
          Stitches Performance Test
        </h1>
        <p style={{ margin: '0.5rem 0 0 0', fontSize: '1.2rem', opacity: 0.9 }}>
          Testing Stitches CSS-in-JS performance with stress testing
        </p>
      </div>
