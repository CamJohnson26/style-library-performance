...
function App() {
  ...
  const exportMetrics = () => {
    ...
    link.href = url;
    link.download = `cva-performance-metrics-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.json`;
    ...
    <div>
      <div style={{ 
        ...
        <h1 style={{ margin: 0, fontSize: '2.5rem', fontWeight: 'bold' }}>
          CVA Performance Test
        </h1>
        <p style={{ margin: '0.5rem 0 0 0', fontSize: '1.2rem', opacity: 0.9 }}>
          Testing CVA (Class Variance Authority) + Tailwind CSS performance with stress testing
        </p>
      </div>
