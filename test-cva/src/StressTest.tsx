  ...
  const generateItems = useCallback(() => {
    ...
    for (let i = 0; i < componentCount; i++) {
      newItems.push({
        id: i,
        title: `CVA Item ${i + 1}`,
        description: `This is a CVA test item with ID ${i + 1} for performance testing`,
    ...
    <div className="max-w-6xl mx-auto p-5">
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-5 mb-8 text-white shadow-lg">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-semibold">Performance Stress Test (CVA + Tailwind)</h2>
          <div className="flex gap-3">
            <button 
              className={cn(
                "bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-medium transition-all duration-200",
            ...
            <button 
              className={cn(
                "bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-6 py-2 rounded-lg font-medium transition-all duration-200",
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-8">
        {items.map(item => (
          <div 
            key={item.id}
            className={cn(
              "bg-white rounded-xl p-5 cursor-pointer transition-all duration-300 hover:shadow-lg hover:-translate-y-1 border-2 relative overflow-hidden",
              item.isActive 
